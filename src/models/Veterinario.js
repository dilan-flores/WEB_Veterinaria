// Importación del Schema y del modelo
import {Schema, model} from 'mongoose'
// Encriptar el password del usuario
import bcrypt from "bcryptjs"


// Creación del Schema
    // En base de datos relacional se denimina tablas
const veterinarioSchema = new Schema({
    nombre:{
        type:String,
        require:true,
            // Borrar todos los espacios en blanco de alrededor de la palabra
        trim:true
    },
    apellido:{
        type:String,
        require:true,
        trim:true
    },
    direccion:{
        type:String,
        trim:true,
        default:null
    },
    telefono:{
        type:Number,
        trim:true,
        default:null
    },
    email:{
        type:String,
        require:true,
        trim:true,
            // No almacenar correos previamente establecidos
		unique:true
    },
    password:{
        type:String,
        require:true
    },
        // Para verificar si el correo está activo
    status:{
        type:Boolean,
        default:true
    },
    token:{
        type:String,
        default:null
    },
        // Para confirmar el correo y luego iniciar sesión
    confirmEmail:{
        type:Boolean,
        default:false
    }

},{ // Tiempo de creación o actualización de información
    timestamps:true
})

// Método para cifrar el password del veterinario
veterinarioSchema.methods.encrypPassword = async function(password){
    const salt = await bcrypt.genSalt(10)
    const passwordEncryp = await bcrypt.hash(password,salt)
    return passwordEncryp
}

// Método para verificar si el password ingresado es el mismo de la BDD
veterinarioSchema.methods.matchPassword = async function(password){
    const response = await bcrypt.compare(password,this.password)
    return response
}

// Método para crear un token 
veterinarioSchema.methods.crearToken = function(){
    const tokenGenerado = this.token = Math.random().toString(36).slice(2)
    return tokenGenerado
}

export default model('Veterinario',veterinarioSchema)